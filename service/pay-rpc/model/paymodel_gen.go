...
var (
	...
	payRowsExpectAutoSet   = strings.Join(stringx.Remove(payFieldNames, "`id`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`", "`created_at`"), ",")
	payRowsWithPlaceHolder = strings.Join(stringx.Remove(payFieldNames, "`id`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`", "`created_at`"), "=?,") + "=?"
...
type (
	...
	Pay struct {
		Id           int64     `db:"id"`
		PaySn        string    `db:"pay_sn"`        // 支付流水号
		UserId       int64     `db:"user_id"`       // 用户ID
		OrderId      string    `db:"order_id"`      // 订单ID
		BuyerAccount string    `db:"buyer_account"` // 下单用户账号
		Price        float64   `db:"price"`         // 支付总金额
		Subject      string    `db:"subject"`       // 支付标题
		Status       int64     `db:"status"`        // 支付状态，0表示未支付，1表示已支付，2表示支付失效，默认为0
...
func (m *defaultPayModel) Insert(ctx context.Context, data *Pay) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.PaySn, data.UserId, data.OrderId, data.BuyerAccount, data.Price, data.Subject, data.Status)
	}, payIdKey)
	return ret, err
}
...
func (m *defaultPayModel) Update(ctx context.Context, data *Pay) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.PaySn, data.UserId, data.OrderId, data.BuyerAccount, data.Price, data.Subject, data.Status, data.Id)
	}, payIdKey)
	return err
}
