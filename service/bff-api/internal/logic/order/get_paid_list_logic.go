...
type GetPaidListLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetPaidListLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetPaidListLogic {
	return &GetPaidListLogic{
...
func (l *GetPaidListLogic) GetPaidList(req *types.GetOrderListRequest) (resp *types.GetPaidOrderListResponse, err error) {
	resp = new(types.GetPaidOrderListResponse)
	userId, err := json.Number(fmt.Sprintf("%v", l.ctx.Value("userId"))).Int64()
	rpcResp, err := l.svcCtx.OrderRPC.GetPaidOrderList(l.ctx, &pb.GetOrderListRequest{
		Page:   req.Page,
		Size:   req.Size,
