...
type (
	...
	Comment struct {
		...
		FilmId     int64     `db:"film_id"` // 电影ID
		Content    string    `db:"content"` // 内容
		Score      float64   `db:"score"`   // 影片评分
...
func (m *defaultCommentModel) Insert(ctx context.Context, data *Comment) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.UserId, data.FilmId, data.Content, data.Score)
	}, commentIdKey)
	return ret, err
}
...
func (m *defaultCommentModel) Update(ctx context.Context, data *Comment) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.UserId, data.FilmId, data.Content, data.Score, data.Id)
	}, commentIdKey)
	return err
}
